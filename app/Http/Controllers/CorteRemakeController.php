...
class CorteRemakeController extends Controller
{
    ...
    {
        try {
            $clientes = clientes::select('clientes.nombre', 'clientes.corte', 'clientes.servidor', 'clientes.ip')
            ...
            if ($clientes->isEmpty()) {
                return response()->json(['message' => 'No hay clientes para suspender']);
            }
        ...
        try {
            ...
            if ($API->connect($ip, env('API_USER'), env('API_PASSWORD'))) {
                foreach ($request->clientes as $cliente) {
                    ...
                    $API->write("=comment={strtoupper({$cliente['nombre']} dia de corte {$cliente['corte']})", true);
                    $READ = $API->read(false);
                }
            } else {
