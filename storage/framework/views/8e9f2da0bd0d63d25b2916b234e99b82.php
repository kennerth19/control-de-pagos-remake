...
<head>
    ...
    <link href="<?php echo e(asset('css/inventario.css')); ?>" rel="stylesheet">
...
<script>var msg = 'Cargando datos';</script>
    ...
    <?php if(session('noEditado') == 'ok'): ?> 
        <script>
            msg = '¡Sin cambios en la categoría!';
        </script>
    <?php endif; ?>

    <?php if(session('entrada') == 'ok'): ?> 
        <script>
            msg = '¡Productos agregado!';
        </script>
    <?php endif; ?>

    <?php if(session('salida_otros') == 'ok' || session('salida') == 'ok'): ?> 
        <script>
            msg = '¡Salida registrada!';
        </script>
    <?php endif; ?>

    <?php if(session('adulterado') == 'ok'): ?> 
        <script>
            msg = '¡Formulario adulterado, enviando reporte al admin!';
        </script>
    <?php endif; ?>

    <?php if(session('categoria_c') == 'ok'): ?> 
        <script>
    ...
    <div id="crear_categoria">
        <form action="<?php echo e(route('store')); ?>" method="POST" enctype="multipart/form-data" id="categoria_form">
            <?php echo csrf_field(); ?>
    ...
    <script>
        var usuarios = <?php echo json_encode($usuarios, 15, 512) ?>;

        if(<?php echo e($totalCategoria); ?> > 0){
            setTimeout(() => {
                main_function(msg);
            }, 500);
        }else{
...
<?php echo $__env->make('side_bar.side_bar', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\control_de_pago_remake\resources\views/inventario/inventario.blade.php ENDPATH**/ ?>