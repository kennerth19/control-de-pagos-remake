async function evento_fecha(int) {
    try {
        ...
        //Total con resto 
        await fetch(`/control_de_pago_remake/public/evento/${fecha}`)
            .then(response => response.json())
            .then(function (response) {
                let total = document.getElementById('total');
                let cantidad_de_eventos = document.getElementById('cantidad_de_eventos');

                if (response[1][0]['D'] == null) { response[1][0]['D'] = 0; }

                if (response[1][0]['BS'] == null) { response[1][0]['BS'] = 0; }

                if (response[1][0]['PM'] == null) { response[1][0]['PM'] = 0; }

                if (response[1][0]['EU'] == null) { response[1][0]['EU'] = 0; }
        ...
        //Total sin resto
        await fetch(`/control_de_pago_remake/public/evento_sr/${fecha}`)
            .then(response => response.json())
            .then(function (response) {
                let total = document.getElementById('total_sin_resto');

                if (response[0][0]['D'] == null) { response[0][0]['D'] = 0; }

                if (response[1][0]['BS'] == null) { response[1][0]['BS'] = 0; }

                if (response[2][0]['EU'] == null) { response[2][0]['EU'] = 0; }

                if (response[3][0]['PM'] == null) { response[3][0]['PM'] = 0; }
        ...
        const response_1 = await fetch('/control_de_pago_remake/public/evento/' + fecha)
            .then(response => response.json())
            .then(data => {
        ...
        if (ev == "" && d + bs + pm + eu == 0) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    fetch(`/control_de_pago_remake/public/evento/agregar/${ev}/${d}/${bs}/${pm}/${eu}/${fec}`, { method: 'POST' })
                        .then(function (response) {
                            Swal.fire({
...
const borrar_evento = async function borrar_evento(id) {
    try {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch(`/control_de_pago_remake/public/evento/eliminar_evento/${id}`)
                    .then(function () {
                        Swal.fire({
                            title: "Eliminado!",
                            icon: "success",
...
const check = async function check(id) {
    ...
    try {
        ...
        const response_4 = await fetch(`/control_de_pago_remake/public/evento/check_evento/${id}/${check}`)
            .then(function () {
                Swal.fire({
                    position: "top-end",
                    icon: "success",
...
function evento_log() {
    window.open('/control_de_pago_remake/public/evento_log', '_blank');
}